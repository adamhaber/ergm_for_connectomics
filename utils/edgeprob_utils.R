tergmprepare_custom <- function(formula, offset = TRUE, blockdiag = FALSE, 
                                verbose = TRUE) {
...
edgeprob_custom <- function (object, verbose = FALSE) {
  ...
  l <- tergmprepare_custom(formula = getformula(object), offset = FALSE,
...
indep_lp_custom <- function(m, g) {
  m$coef <- ifelse(is.infinite(m$coef),-10,m$coef)
  t <- edgeprob_custom(m, verbose = T)
